...
namespace Client.ViewModels //view model for the main directory view
{
    ...
    #region properties
    public class MyTaggedFilesViewModel : Screen
    {
        ...
        public MyTaggedFilesViewModel()
        {
            Tags.XMLFile.init();
            var children = DirStructureModel.GetLogicalDrives(); //get the logical drives - each one's name and path
            this.Items = new BindableCollection<MyViewModel>(children.Select(drive => new MyViewModel(drive.FullPath, DirEnumModel.Drive))); //for each drive create myviewmodel object
        ...
        */
        ...
        {
            if (selectedItem.Type == DirEnumModel.File)
            {
